...
frappe.ui.form.on("Aiko Dashboard Session", {
    refresh(frm) {
        if (!frm.is_new()) {
            aiko_dashboard_load_conversation(frm);
        }
    },
});

/**
 * Fetches all Aiko Dashboard Messages for this session,
 * renders styled HTML blocks, and populates the child table.
 */
function aiko_dashboard_load_conversation(frm) {
    frappe.call({
        ...
        callback(r) {
            ...
            if (messages.length === 0) {
                ...
                const blocks = messages.map((msg, i) => aiko_dashboard_render_block(msg, i + 1)).join("");
                container.html(
            ...
            messages.forEach((msg, i) => {
                let row = frappe.model.add_child(frm.doc, "Aiko Dashboard Session Message", "messages");
                row.idx_display   = i + 1;
                row.role          = msg.role;
                row.content       = msg.content;
                row.llm_provider  = msg.llm_provider;
 ...
 */
function aiko_dashboard_render_block(msg, index) {
    ...
    // Provider/model badge — only show for assistant, only if set
    const providerBadge = (!isUser && (msg.llm_provider || msg.llm_model))
        ? `<span style="
                background:#e6f7ee;
