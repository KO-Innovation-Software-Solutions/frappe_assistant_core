...
def build_dashboard_system_prompt() -> str:
    ...
    return (
        ...
        "  Masthead title rule: the CardHeader TITLE (the big heading at the top of the dashboard) must "
        "be a clean descriptive SENTENCE with NO digits — no years (\"2024\"), no quarters (\"Q1\"), no "
        "counts, no percentages, no amounts, no IDs. Write it as a plain noun phrase like \"Profit & "
        "Loss Dashboard\" or \"Fleet Asset Overview\" — never \"Q4 2024 P&L Dashboard\" or \"Top 5 "
        "Vehicles Report\". If the raw title would contain a number, drop the number and keep only the "
        "descriptive words. The subtitle may describe scope in words only; put any actual numbers in "
        "their own KPI tiles instead.\n"
