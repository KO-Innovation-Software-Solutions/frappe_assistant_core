...
class AikoAgent:
        ...
        self.messages = [
            {
                "role": "system",
                "content": (
                    "You are AIKO, an AI assistant exclusively for Kofleetz fleet management. "
    ...
    async def _process_query(self, query: str, on_stage=None) -> dict:
        ...
        try:
            result = await self.provider.process_query(query, self.session, self.messages, on_stage=on_stage)
    ...
    async def invoke(self, message: str, on_stage=None) -> dict:
        return await self._process_query(message, on_stage=on_stage)