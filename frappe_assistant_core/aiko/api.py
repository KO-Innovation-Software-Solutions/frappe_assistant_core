...
def _get_active_llm_info(settings=None):
    """Return (provider, model) from Assistant Core Settings."""
...
def _save_file_to_frappe(file_name: str, file_type: str, file_data: str, user: str) -> str:
    """
    Save a base64-encoded file into Frappe's File DocType (private)
    so that the extract_file_content MCP tool can locate it by file_url.
...
# ------------------------------------------------------------------ #
# Plain-text chat  (queued, async, with cancellation + realtime)
# ------------------------------------------------------------------ #
...
async def run_chat_job(message: str, thread_id: str, user: str, request_id: str):
    """Background worker: runs the agent, emits stage events, saves result."""
    ...
    try:
        ...
        if _is_cancelled(request_id):
            ...
            frappe.logger().info(f"AIKO request {request_id} cancelled -- skipping assistant save.")
            return
        ...
        _save_message(session.name, role="assistant", content=response_text,
...
# ------------------------------------------------------------------ #
# File chat  (queued, async -- added from friend's code)
# ------------------------------------------------------------------ #
 
@frappe.whitelist()
def chat_with_file(
    message: str, thread_id: str,
    file_name: str, file_type: str, file_data: str,
    request_id: str = None,
):
    """
    Chat endpoint that accepts a file (image, PDF, document) as base64.
    Queues a background job just like plain-text chat so the frontend
    gets the same realtime stage/done events.
    """
    if not frappe.session.user or frappe.session.user == "Guest":
        frappe.throw(_("Authentication required"))
    ...
    try:
        frappe.enqueue(
            "frappe_assistant_core.aiko.api.run_file_chat_job_sync",
            queue="default",
            timeout=300,
            message=message,
            thread_id=thread_id,
...
async def run_file_chat_job(
    ...
    try:
        ...
        if len(raw_bytes) > 20 * 1024 * 1024:
            frappe.publish_realtime(
                ...
                message={
                    ...
                    "success": False, "error": "File too large. Maximum 20 MB allowed.",
                },
                user=user,
            )
            return
        ...
        _save_message(session.name, role="user",
...
def run_file_chat_job_sync(
    ...
    """Sync entry point for Frappe's background worker (file uploads)."""
