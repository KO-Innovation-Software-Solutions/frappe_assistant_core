...
class FileHandler:
    ...
    async def update_doctype(
        ...
        def _regex_extract(text: str, dt: str) -> dict:
            ...
            # Regex allows optional spaces between parts (e.g. "KL 43 CA 1234" or "KL43CA1234")
            _REG_PAT = r'([A-Z]{2}\s?\d{1,2}\s?[A-Z]{1,3}\s?\d{1,4})'
            regn_match = re.search(
                r'(?:Vehicle\s*Reg(?:n|istration)?\.?\s*No\.?\s*[:\-]?\s*|'
                r'Reg(?:n|istration)?\s*No\.?\s*[:\-]?\s*|'
                r'Vehicle\s*No\.?\s*[:\-]?\s*|'
                r'Vehicle\s*Number\s*[:\-]?\s*|'
                r'Veh\.?\s*No\.?\s*[:\-]?\s*)' + _REG_PAT,
                text, re.IGNORECASE
            )
            if regn_match:
                fields["vehicle"] = re.sub(r'\s+', '', regn_match.group(1)).upper()
            else:
                ...
                if bare_match:
                    fields["vehicle"] = re.sub(r'\s+', '', bare_match.group(1)).upper()
            ...
            if dt == "Service Entry":
                ...
                qty_match = re.search(
                    r'(?:VOLUME|Qty|Quantity|Litres?|Liters?)\s*[:\-]?\s*([\d]+\.?\d*)',
                    text, re.IGNORECASE
                )
                if qty_match:
                ...
                fuel_type_match = re.search(
                    r'(?:PRODUCT|Fuel\s*Type)\s*[:\-]?\s*(\w+)',
                    text, re.IGNORECASE
                )
                if fuel_type_match:
                    fields["fuel_type"] = fuel_type_match.group(1).capitalize()

                # Vendor: fuel station name from letterhead (first non-blank line)
                vendor_name = None
