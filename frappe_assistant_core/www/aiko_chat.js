/* ═══════════════════════════════════════════════════════════════════════════
   AIKO Chat — Full-page web app (www/ version)
   No frappe.pages wrapper — exposed as window.AikoChatPage.init(wrapper)
   ═══════════════════════════════════════════════════════════════════════════ */
...
window.AikoChatPage = {
    ...
    if (messageCount >= 20) {
        ...
        if (!$('#aiko-limit-banner').length) {
            $('.aiko-page-input-area').before(`
                <div id="aiko-limit-banner" class="aiko-limit-banner">
                    <p>This chat has reached 10 messages.<br>Start a new session to continue.</p>
        ...
        function startNewChat() {
            ...
            $('#aiko-sidebar-sessions .aiko-session-item').removeClass('active');
                ...
                $('#aiko-page-new-chat, #aiko-page-new-chat-topbar').on('click', function () { startNewChat(); });
        ...
        function renderMarkdown(text) {
            ...
            // A table is: one header row, one separator row (|---|), then data rows
            const tables = [];
            text = text.replace(/^(\|.+\|\n)([ \t]*\|[-| :]+\|\n)((?:[ \t]*\|.+\|\n?)+)/gm,
    ...
    pageScrollToBottom();
        ...
        $('#aiko-page-stop-btn').on('click', function () {
            ...
            if (pendingXhr && pendingXhr.abort) pendingXhr.abort();
            removePageThinking();
...
if (sessionFromWidget) {
    ...
    if (wasThinking) {
        ...
        // Wait for loadSession to finish rendering, then append pending state
        setTimeout(function () {
